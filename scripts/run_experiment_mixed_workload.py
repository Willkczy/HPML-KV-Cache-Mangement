...
def parse_args():
    ...
    parser.add_argument("--num_requests", type=int, default=None,
                        help="Cap total number of trace requests.")
...
def main():
    ...
    elif args.num_requests is not None:
        trace = trace[:args.num_requests]
        print(f"[runner] --num_requests cap — using {len(trace)} requests.")
    ...
    for i, req in enumerate(trace):
        ...
        use_rouge = source_dataset == "govreport"
        ...
        prefill_kv_mb = output.metadata.get(
            "prefill_kv_memory_mb",
            output.metadata.get("prefill_peak_kv_memory_mb", output.peak_kv_memory_mb),
        )
    ...
    for bname in bucket_names:
        ...
        # Quality per bucket
        mcq_recs = [r for r in b_records if r["dataset"] in ("mmlu", "longbench")]
        rouge_recs = [r for r in b_records if r["dataset"] == "govreport"]

        if mcq_recs:
            n_correct = sum(r["correct"] for r in mcq_recs)
            bucket_stats["accuracy"] = round(n_correct / len(mcq_recs), 4)
        if rouge_recs:
    ...
    # Overall quality
    all_mcq = [r for r in valid_records if r["dataset"] in ("mmlu", "longbench")]
    all_rouge = [r for r in valid_records if r["dataset"] == "govreport"]
    if all_mcq:
        n_correct = sum(r["correct"] for r in all_mcq)
        summary["overall_accuracy"] = round(n_correct / len(all_mcq), 4)
    if all_rouge:
    ...
    if all_mcq:
        print(f"  MCQ accuracy:      {summary['overall_accuracy']:.1%} ({len(all_mcq)} samples)")
    if all_rouge:
